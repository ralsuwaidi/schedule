...
EVENTBRITE_TOKEN = "T7HVN5UXVVKCU7ORBY57"
EVENTBRITE_ORGANIZATION_ID = "463481795527"
...
class Eventbrite:
    ...
    def create_event(cls, chq_event):
        ...
        print(publish_event_response)

        return event_id

    def post_basic_info(self) -> dict:
        # TODO: check best way to modify the request's body
        url = f'{Eventbrite.BASE_URL}/organizations/{EVENTBRITE_ORGANIZATION_ID}/events/'
...
if __name__ == '__main__':
    class chq_event:
        def __init__(self):
            self.name="First Update of event using new architect"
            self.start=datetime.now(tz=timezone.utc)+timedelta(minutes=1)
            self.end=self.start
            self.summary="This an updated summary"
            self.description="THis an updated description"
            self.requirements="pcr required"
            self.event_link="http://"
            self.event_location="emirates towers"
            self.capacity=40




    event_details = chq_event()

    event_id = Eventbrite.create_event(event_details)
